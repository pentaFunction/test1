...
EventContext  = FairyGUI.EventContext
EventListener = FairyGUI.EventListener
FairyGUI_EventDispatcher = FairyGUI.EventDispatcher
InputEvent = FairyGUI.InputEvent
NTexture = FairyGUI.NTexture
Container = FairyGUI.Container
Image = FairyGUI.Image
Stage = FairyGUI.Stage.inst
Controller = FairyGUI.Controller
GObject = FairyGUI.GObject
GGraph = FairyGUI.GGraph
GGroup = FairyGUI.GGroup
...
PlayCompleteCallback = FairyGUI.PlayCompleteCallback
AlignType = FairyGUI.AlignType
AutoSizeType = FairyGUI.AutoSizeType
FillType = FairyGUI.FillType;
StageCamera = FairyGUI.StageCamera;
ListItemRenderer = FairyGUI.ListItemRenderer;
ListItemProvider = FairyGUI.ListItemProvider
TransitionHook =  FairyGUI.TransitionHook
ChildrenRenderOrder = FairyGUI.ChildrenRenderOrder
--Tween
GTween = FairyGUI.GTween
GTweener = FairyGUI.GTweener
EaseType = FairyGUI.EaseType
...
function fgui.register_extension(url, extension)
    ...
    local ext = registTable[url];
    if ext ~= nil and ext == extension  then
        return;
    end
    if extension ==nil then
        error(url.."    extension is nil")
        return;
    end
    registTable[url] = extension;
    FairyGUI.LuaUIHelper.SetExtension(url, typeof(base), extension.Extend)
end

function fgui.check_field(self, field)
    local typeStr = type(self)
...
---检测协议里字段是否存在
function fgui.check_proto_field(self, field)
...
function fgui.extension_class(base)
    ...
    if UseLuaNullErr then
        o.testDispose = function(this)

            GameLoopManager:TestDispose(this,true);
            EventDispatcher:TestDispose(this,true);
            local test = tolua.getpeer(this)
            local str = "";
            local str1 = ""
            local typeStr;
            for k,v in pairs(test) do
                if k ~= "base" then
                    ...
                    elseif typeStr ~= "boolean" and typeStr ~= "string" and typeStr ~= "number" then
                        str =str.. "self."..k.."= nil;\n"
                    end
                end
            end
            if  str ~= "" or str1 ~= "" then
                print(str1)
                print(str)
                local showName = this.name;
                if nil == showName then
                    showName = "---";
                end
                LuaHelper.RreportError(debug.traceback(string.format("错误：你的扩展%s有引用没置nil,顺便往上看看输出有没有监听也没移除,你也可以复制上面那条黏贴过去",showName),2));
            end
        end
    end
    return o
...
function fgui.remove_timer(func, obj)
    local delegate,mapping,func = fgui_internal.GetDelegate(func, obj, false)
...
local EventDispatcher_mt = getmetatable(FairyGUI_EventDispatcher)
...
--将AddOnClickToChild和RemoveOnClickFromChild重新进行定义，以适应lua的使用习惯
local _GComponentExtensions = getmetatable(GComponent)
local _AddOnClickToChild = rawget(_GComponentExtensions, 'AddOnClickToChild')
local _RemoveOnClickFromChild = rawget(_GComponentExtensions, 'RemoveOnClickFromChild')
local function AddOnClickToChild(component, child, func, obj)
    local delegate = fgui_internal.GetDelegate(func, obj, true)
    _AddOnClickToChild(component, child, delegate)
end
local function RemoveOnClickFromChild(component, child, func, obj)
    local delegate,mapping,func = fgui_internal.GetDelegate(func, obj, false)
    if delegate ~= nil then
        _RemoveOnClickFromChild(component, child, delegate)
    end
    if func ~= nil then
        mapping[func] = nil;
    end
end
rawset(_GComponentExtensions, 'AddOnClickToChild', AddOnClickToChild)
rawset(_GComponentExtensions, 'RemoveOnClickFromChild', RemoveOnClickFromChild)
